    ...
    >
      <Box>
        {typeof avatarIcon === 'string' ? (
          <AvatarIcon
            ...
            color={weight === 1 ? Color.warningDefault : Color.primaryDefault}
            backgroundColor={
              weight === 1 ? Color.warningMuted : Color.primaryMuted
            }
          />
        ) : (
          avatarIcon
      ...
      <Box>
        <Tooltip html={<div>{description}</div>} position="bottom">
          <Icon
            color={
              weight === 1 ? IconColor.warningDefault : IconColor.iconMuted
            }
            name={weight === 1 ? ICON_NAMES.DANGER : ICON_NAMES.INFO}
            size={ICON_SIZES.SM}
          />
        </Tooltip>
      </Box>
